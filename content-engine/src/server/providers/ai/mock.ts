...
const MOCK_CHECK = [{ item: "매수·매도 권유 표현이 0개인가", pass: true }, { item: "숫자는 facts.display 그대로 인용했는가", pass: true }];
...
function mockThreads(ctx: Record<string, unknown>): ThreadsBody[] {
  ...
  const replyText = !includeLink && cta.url ? `정리한 계산표는 여기서 볼 수 있습니다 → ${cta.url}` : "";
...
function mockBlog(ctx: Record<string, unknown>): BlogBody {
  ...
  return {
    tags: calc ? ["배당금계산", "배당수익률", "월배당", "배당주", "재테크", "투자", "경제", "월배당만들기", "배당투자금", "노후준비"] : ["배당", "배당투자", "재테크", "투자", "경제", "주식", "월배당"],
    selfCheck: MOCK_CHECK,
