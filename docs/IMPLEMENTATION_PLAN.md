...
| 토큰 절감 | ① 출력 다이어트: 목차·출처·기준일·면책·altText·색상은 프로그램이 채움(모델 스키마에서 제외), 제목 후보 3개, 블로그 기본 1,500자 ② 프롬프트 캐싱: system을 [공통 규칙][Master·브랜드·제품(cache_control)][채널 지시]로 분리 ③ Message Batches: 생성 방식 "배치"(50% 단가), `content.generate.collect` Job이 2분 뒤부터 5분 간격으로 회수, 화면의 "지금 확인"으로 즉시 회수 | 비용의 70%가 출력 토큰. 배치가 가장 큰 절감 |
| Threads 편수 | 기본 1편(훅 각도 5종 중 하나를 고르는 `angle` 필드), 옵션으로 2~3편 | 3편 중 1편만 쓰이는 낭비 제거 |
| 기본 채널 | 생성 패널에서 Shorts 기본 해제 | 사용자가 자동화 대상을 3채널로 한정 |
